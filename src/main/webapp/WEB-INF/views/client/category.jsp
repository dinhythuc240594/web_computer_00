...
<%
    ...
    if (selectedBrand != null) {
        if (pageTitle.equals("Tất cả sản phẩm")) {
            ...
            pageTitle += " - " + selectedBrand.getName();
        }
    }
%>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .category-list li:last-child, .brand-list li:last-child {
            margin-bottom: 0;
        }
    </style>
</head>
                            ...
                            <select name="brandId" id="brandId">
                                ...
                                <%
                                    for (BrandDAO brand : brands) {
                                        boolean isSelected = brand.getId() == selectedBrandId;
                                %>
                                <option value="<%= brand.getId() %>" <%= isSelected ? "selected" : "" %>>
                                    <%= brand.getName() %>
                                </option>
                                <%
                                    }
                        ...
                        <div class="category-widget sidebar-widget">
                            ...
                            <div class="widget-content">
                                <ul class="category-list">
                                    <li>
                                        <%
                                            // Khi click "Tất cả", reset cả categoryId và brandId, chỉ giữ keyword nếu có
                                            String allUrl = contextPath + "/category";
                                            if (searchKeyword != null && !searchKeyword.isBlank()) {
                                                allUrl += "?keyword=" + java.net.URLEncoder.encode(searchKeyword, "UTF-8");
                                            }
                                            // Chỉ active khi không có category nào được chọn VÀ không có brand nào được chọn
                                            boolean isAllActive = selectedCategoryId == 0 && selectedBrandId == 0;
                                        %>
                                        <a href="<%= allUrl %>" 
                                    ...
                                    <%
                                        for (CategoryDAO category : categories) {
                                            // Chỉ active category khi categoryId được chọn VÀ không có brand nào được chọn
                                            boolean isActive = category.getId() == selectedCategoryId && selectedBrandId == 0;
                                            // Khi click vào category, chỉ filter theo categoryId, không giữ brandId
                        ...
                        <div class="brand-widget sidebar-widget">
                            ...
                            <div class="widget-content">
                                <ul class="brand-list">
                                    <!-- <li>
                                        <%
                                            // Khi click "Tất cả thương hiệu", reset brandId và giữ keyword nếu có
                                            String allBrandsUrl = contextPath + "/category";
                                            if (searchKeyword != null && !searchKeyword.isBlank()) {
                                                allBrandsUrl += "?keyword=" + java.net.URLEncoder.encode(searchKeyword, "UTF-8");
                                            }
                                            // Chỉ active khi không có brand nào được chọn VÀ không có category nào được chọn
                                            // Nhưng ưu tiên "Tất cả danh mục" nếu cả hai đều 0, nên "Tất cả thương hiệu" không active
                                            boolean isAllBrandsActive = false; // Không bao giờ active để tránh conflict
                                        %>
                                        <a href="<%= allBrandsUrl %>" 
                                           class="sidebar-nav-link <%= isAllBrandsActive ? "active" : "" %>">
                                            Tất cả thương hiệu
                                        </a>
                                    </li> -->
                                    <%
                                        for (BrandDAO brand : brands) {
                                            // Chỉ active brand khi brandId được chọn VÀ không có category nào được chọn
                                            boolean isActive = brand.getId() == selectedBrandId && selectedCategoryId == 0;
                                            // Khi click vào brand, chỉ filter theo brandId, không giữ categoryId
                                            String brandUrl = contextPath + "/category?brandId=" + brand.getId();
                                            if (searchKeyword != null && !searchKeyword.isBlank()) {
